...
def main():
    ...
    batchsize_train= 48
    root_eval = './data/matched_face'
    list_eval = './data/txt/matched_face_inf.txt'
    batchsize_eval= 64
    ...
    #model = networks_4emo.resnet18_at(at_type=at_type)
    saved_model= './model/self-attention_4_emo_model'
    #model.load_state_dict(checkpoint['state_dict'])
    model = load_4emo.model_parameters(_structure, saved_model )
    ''' Loss & Optimizer '''
...
def val(val_loader, model, at_type):
    ...
    with torch.no_grad():
        ...
        #save pred res
        #torch.save(pred_score, 'pred_score.pt')
        pred_res = pred_score.cpu().numpy()
        print('res:',pred_res,pred_res.shape)
        numpy.savetxt("res.csv",pred_res,delimiter=",")
